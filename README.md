...
| 问题 | 可能原因 | 解决方法 |
|------|----------|----------|
| OLED 显示汉字乱码 | 字模数据格式不一致 | 确保 PCtoLCD2002 设置为阴码、逐列式、高位在前 |
| DHT11 一直返回 HAL_ERROR | 确认 DWT 初始化成功 |
| 光敏读数大幅度跳动 | ADC 采样时间不足 / 电源噪声 | 增大采样时间至最大（239.5 周期）；增加软件平均滤波 |
| 蜂鸣器不响 | 引脚配置错误或驱动电流不足 | 检查 GPIO 输出模式；使用三极管放大驱动 |
